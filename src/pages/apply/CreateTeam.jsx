...
export default function CreateTeam() {
    ...
    const redirectingRef = useRef(false);              // 👈 prevent multi-redirects
    ...
    // auth & redirect if already in a team
    useEffect(() => {
        let alive = true;
        (async () => {
            ...
            if (!s) {                     // don't hard-redirect; just stop checking and show UI message
                setChecking(false);
                return;
            }

            // Use RPC for consistent RLS behavior
            const { data: teamId, error } = await supabase.rpc('get_my_team_id');
            if (!alive) return;

            if (!error && teamId && !redirectingRef.current) {
                redirectingRef.current = true;
                nav('/team', { replace: true }); // 👈 redirect once
                return;
            }
    ...
    async function submit(e) {
        ...
        const { error } = await supabase.rpc('create_team_and_join', {
            ...
            // store institute even for non-students (company name)
        ...
        <div className="c_team-wrap">
            <div className="c_team-card">
                ...
                <form className="c_team-form" onKeyDown={onFormKeyDown} onSubmit={submit} autoComplete="off">
                    ...
                    {step === 2 && (
                        <section className="c_team-section">
                            ...
                            <div className="c_team-grid">
                                ...
                                {/* Institute always visible; required only if isStudent */}
                    ...
                    {/* STEP 4 — Review (with its own submit button) */}
                    {step === 4 && (
                        <section className="c_team-section">
                            ...
                            {/* Submit button inside review step */}
                    ...
                    {/* Bottom nav: Back / Next (no submit here on step 4) */}
